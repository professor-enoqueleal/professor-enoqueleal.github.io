+++
title = "LAB 12 - Deploying to AWS with Elastic Beanstalk and RDS"
description = "Step-by-step tutorial to deploy the Spring Boot application on AWS using Elastic Beanstalk via the web console, with automatic RDS configuration and H2DB support for local development."
date = 2026-05-14
draft = false
author = "Enoque Leal"
...
In this lab vamos fazer o deploy da aplicação **carstore-spring-boot** na AWS usando o **Elastic Beanstalk**. O Elastic Beanstalk é um serviço gerenciado que facilita o deploy de aplicações web, automaticamente provisionando recursos como load balancers, Auto Scaling groups e bancos de dados.
...
## Task 1: Preparando o projeto para múltiplos ambientes

### Parte 1: Configuração local (H2DB)

Vamos manter a configuração atual para desenvolvimento local. Open the file `src/main/resources/application.properties` e mantenha as configurações do H2:
...
>
> **IMPORTANTE - MySQL 8.0+**: O parâmetro `allowPublicKeyRetrieval=true` é OBRIGATÓRIO para MySQL 8.0+ (incluindo a versão 8.4.8 usada in this lab). O MySQL 8.0+ usa o método de autenticação `caching_sha2_password` que requer este parâmetro para funcionar corretamente com o driver JDBC. Sem ele, você receberá o erro: "Public Key Retrieval is not allowed".
>
...
## Task 2: Gerando o JAR para deploy
...
## Task 3: Criando o ambiente no Elastic Beanstalk (Wizard Web)
...
2. Na barra de busca, type "Elastic Beanstalk" e selecione o serviço;
3. Clique em "Create environment" (o wizard será iniciado diretamente).

---

## Task 4: Configurando o Step 1 - Configure environment Info
...
## Task 5: Configurando o Step 2 - Configure service access
...
- **EC2 key pair**: Deixe em branco
  ...
  - Não teremos acesso SSH in this lab
...
## Task 6: Configurando o Step 3 - Set up networking, database, and tags
...
## Task 7: Revisando e criando o ambiente (Step 4 e Step 5)
...
## Task 10: Gerenciando variáveis de ambiente (IMPORTANTE)
...
## Task 11: Acessando o RDS diretamente (opcional)
