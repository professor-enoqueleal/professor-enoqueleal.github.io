+++
title = "LAB 7 - Creating the login feature"
description = "This lab aims to present a basic approach on how to implement a login feature using username and password!"
date = 2025-05-09
draft = true
author = "Enoque Leal"
...
## Task 1: Criando a tabela USR (Usuário)

One of the characteristics of the system **Car Store** is its administrative area that should only be accessed by authorized users. To do this ser possível, será necessário a implementação de uma camada de autenticação.

1 - Create a new user table. With the application running *(tomcat7:run)* navegue até a console de gerênciamento do *H2 DB* através do link: *http://localhost:8080/console*
...
3 - After logging in, you can create your segunda tabela. Você deverá criar uma tabela chamada **USER**. To do this utilize o comando **SQL** a seguir:

```sql
CREATE TABLE USR (ID INT PRIMARY KEY AUTO_INCREMENT, USERNAME VARCHAR(255), PASSWORD VARCHAR(255));
```

4 - After the table is created, let's create our first user manually. To do this, utilize o seguinte comando SQL:

```sql
INSERT INTO USR (USERNAME, PASSWORD) VALUES ('your-user', 'your-password')
```

5 - Faça uma revisão tudo que foi feito até aqui, to ensure that the table was created and that the first user registration (INSERT) worked correctly.

---

## Task 2: Creating the Model and DAO classes responsible for retrieving login credentials

1 - The first step is criar a classe User. To do this, In IntelliJ o pacote *(package)* model que fica no diretório: car-store-guide/app/src/main/java/br.com.carstore.model, right-click e select the option New / Java Class e type o nome **User** e press the **ENTER**. 

After the class **User** has been created, let's add duas propriedades do tipo String com o modificador de acesso private. The properties are **username** e **password**.

We need to implement the accessor method (getters) and an overloaded constructor that receives these two new properties.
...
2 - The second step is to criar a DAO chamada **UserDao**. This class será a responsável por fazer a consulta no banco de dados e recuperar as credenciais do usuários armazenadas no banco de dados. 

To do this, In IntelliJ navigate to the package *(package)* DAO que fica no diretório: car-store-guide/app/src/main/java/br.com.carstore.dao, right-click e select the option New / Java Class e type o nome **UseDao** e press the **ENTER**.

3 - After the class has been created, let's implement o nosso primeiro método chamado **verifyCredentials()**. This method retorna um boolean e recebe um parâmetro do tipo **User**.

It will be responsible for querying the database based on the username provided by the user through the Web form. 

The resulting code of the initial implementation should be equal to the following:
...
## Task 3: Criando a Servlet responsável por capturar as credenciais de login

1 - Agora com a tabela **USR** criada no banco de dados e a classe DAO properly created, podemos implementar a classe Servlet que receberá as requisições HTTP com as informações de login (username e password).

2 - The first step is criar a classe User. To do this, In IntelliJ o pacote *(package)* model que fica no diretório: car-store-guide/app/src/main/java/br.com.carstore.servlet, right-click e select the option New / Java Class e type o nome **LoginServlet** e press the **ENTER**.
...
No caso de credencial válida, nós adicionamos uma variável a sessão chamada **loggedUser**, as shown in the following code.

```java
req.getSession().setAttribute("loggedUser", username);
```

E depois redirecionamos o usuário para nossa roda find-all-carls, as shown in the following code:

```java
resp.sendRedirect("find-all-cars");
```

Após toda implementação ter sido feia, the resulting code should be equal to the following code:
...
## Task 4: Criando a Servlet responsável pela operação de logout
...
In IntelliJ o pacote *(package)* model que fica no diretório: car-store-guide/app/src/main/java/br.com.carstore.servlet, right-click e select the option New / Java Class e type o nome **LogoutServlet** e press the **ENTER**.
...
2 - resultado final should be equal to the following code:
...
## Task 5: Criando o Filter (filtro) responsável por verificar se o usuário está autenticado ou não
...
To do this, remos criar um novo pacote chamado filter. In IntelliJ navigate to the package *(package)* principal que fica no diretório: car-store-guide/app/src/main/java/br.com.carstore, right-click e select the option New / Package e type o nome **filter** e press the **ENTER**.

2 - Com o pacote filter criado, let's create a nossa classe que chamaremos de **AuthenticationFilter**. To do this, no IntelliJ right-click no pacote filter que acabamos de criar, e select the option New / Java Class e type o nome **AuthenticationFilter** e press the **ENTER**.
...
Após toda a implementação, the resulting code should be equal to the following code:
...
5 - Review everything that has been done so far!

---

## Task 6: Criando a página HTML responsável por capturar as credenciais de login

1 - Com a DAO, Servlet e Filter de login properly createds, chegou o momento de criar a página (jsp) de login.

In IntelliJ navegue até a pasta **webapp*, fica no diretório: car-store-guide/app/src/main/webapp, right-click e select the option New / File e type o nome **login.jsp** e press the **ENTER**.
...
## Task 7: Refatorando a página dashboard.jsp
...
1 - In IntelliJ, navegue até o diretório car-store-guide/app/src/main/webapp, e abra a página **dashbord.jsp**, troca a implementação original:
...
To do this, In IntelliJ, navegue até o package servlet que fica no diretório car-store-guide/app/src/main/br.com.carstore.servlet, e open the class **ListCarServlet.java**.
...
5 - Save all changes **(CTRL + S)** and run your application (tomcat7:run).

Access your application through the link http://localhost:8080 e faça o cadastro de um carro. Repare que a página que é exibida não é mais a página de cadastro e sim a página de login. Esse e o comportamento esperado, porque a página padrão agora é a página **login.jsp**. Para acessar a página **dashboard.jsp** as cre devem ser informadas. 

NOTE: Atenção nesse ponto, pois você deve digitar as mesmas credenciais (username e password) que você incluiu no banco (INSERT) no paço 4. Caso contrário você irá receber um feedback de usuário ou senha inválidos.
