+++
title = "LAB 9 - Protecting the API (REST) Layer with Spring Security and JWT"
description = "Step-by-step guide to configuring security in REST endpoints, using Spring Security in a stateless manner and implementing JSON Web Token (JWT) based authentication."
date = 2026-05-08
draft = false
author = "Enoque Leal"
...
In this lab, iremos configurar o Spring Security para proteger a camada de API da aplicação `carstore-spring-boot`. Ao contrário da camada web (LAB 8), a API será **stateless** (sem sessão) e usará o protocolo de autenticação **JWT (JSON Web Tokens)**.

At the end deste laboratório, você deverá saber:
...
### Task 1: Dependências Adicionais
...
### Task 2: Implementação da Classe LoginRequest (DTO)
...
### Task 3: Refatorando a classe SecurityConfig (Separação Web vs API)

Na classe `SecurityConfig.java`, precisamos criar um novo método filter para a proteção da camada de API (stateless e JWT).

To do this, a baixo do método `filterChain`, crie um novo método chamado `apiFilterChain`, conforme o exemplo a seguir:
...
### Task 4: Criação do JWT Token Service
...
### Task 5: Criação do Controller de Autenticação (API)
...
### Task 6: Criação do Filtro JWT

Agora let's implement o filtro que irá intercepta todas as requisições da API para validar o token antes que a requisição chegue ao `Controller`.
...
### Task 7: Integrando o Filtro no SecurityConfig
...
In this lab, você separou o fluxo de segurança, criando uma camada de API stateless protegida por tokens JWT. Você implementou a geração, validação e aplicação do token em um filtro customizado do Spring Security.
