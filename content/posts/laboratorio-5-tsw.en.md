+++
title = "LAB 5 - Improving the interface with Bootstrap (WebJars)"
description = "This lab presents the step-by-step process to add Bootstrap to the CarStore project with Spring Boot and Thymeleaf, improving the application interface."
date = "2026-04-15"
draft = false
author = "Enoque Leal"
...
In this lab, let's add o **Bootstrap** ao projeto **CarStore**, utilizando o **WebJars**.  
...
## Task 1: Adicionar o WebJar do Bootstrap

No arquivo **`pom.xml`**, add the following dependency:
...
## Task 2: Referenciar os arquivos no HTML
...
Agora, open the file `index.html`, localizado no diretório: `src/main/resources/templates/index.html` e adicione o import do `bootstrap.min.css`, do javascript do `jquery.min.js` e também do `bootstrap.bundle.min.js` conforme exemplo abaixo.
...
## Task 2.1: Ajustando o formulário de Cadastro

Now that we have os imports no nosso formulário de cadastro, vamos fazer uma pequena refatoração, para deixar o estilo da nossa página de cadastro mais agradável utilizando o bootstrap.
...
Agora, nas `divs` que envolvem os campos `label` e `input`, vamos referenciar a classe `mb-3` do bootstrap. This class aplica uma margem inferior (margin-bottom) de acordo com a escala de espaçamentos do Bootstrap. Ela vai fazer com que os campos não fiquem esteticamente colados uns nos outros.
...
## Task 3: Melhorando nossa tela de listagem

Agora vamos melhorar o aspecto visual da nossa página de listagem `dashboard.html`. To do this, open the file `dashboard.html`, localizado no diretório: `src/main/resources/templates/dashboard.html` e adicione o import do `bootstrap.min.css`, do javascript do `jquery.min.js` e também do `bootstrap.bundle.min.js`, assim como foi feito na página index.html na tarefa 2.
...
## Task 4: Criando um menu com Bootstrap e reutilizando com Thymeleaf

Nesta tarefa, let's create um **menu de navegação (navbar)** usando Bootstrap e reaproveitá-lo em várias páginas do projeto **Car Store** utilizando o conceito de **fragmentos do Thymeleaf**.  
