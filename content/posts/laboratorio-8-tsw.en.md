+++
title = "LAB 8 - Implementing the Login feature with Spring Security"
description = "Step-by-step guide to adding form-based authentication to the web layer using Spring Security (starter)."
date = 2026-05-07
draft = false
author = "Enoque Leal"
...
In this lab iremos implementar a funcionalidade de autenticação pora os formulários da camada web da aplicação `carstore-spring-boot`. To do this, iremos utilizar o Spring Security Starter. O foco é permitir que usuários façam login via uma página HTML (Thymeleaf) e que a sessão seja mantida pelo container de sessão do Spring Security.
...
### Task 1: Importando a dependência do Spring Security
...
### Task 2: Criando a classe de configuração do Spring Security
...
### Task 3: Criando um usuário para testes (in-memory)
...
### Task 4: Criando a página de login com Thymeleaf
...
### Task 5: Criando a Controller responsável pelo fluxo de login
...
### Task 6: Validando a implementação
...
In this lab você configurou o Spring Security, protegeu rotas de administração `(/admin/**)` e implementou o fluxo de autenticação por formulário com Thymeleaf.
