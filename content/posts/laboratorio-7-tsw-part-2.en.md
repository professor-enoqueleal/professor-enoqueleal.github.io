+++
title = "LAB 7 - Database integration via JPA - PART 2"
description = "Step-by-step guide to integrating a Spring Boot application with a relational database using JDBC (JdbcTemplate) and JPA (Jakarta Persistence / Hibernate)."
date = 2025-10-16
draft = true
author = "Enoque Leal"
...
In this lab let's add e configurar a persistência na nossa aplicação spring-boot   (`carstore-spring-boot`). O objetivo é apresentar duas abordagens comuns para trabalhar com bancos relacionais em aplicações Spring:

- Uso de JPA (Jakarta Persistence / Hibernate) diretamente via `EntityManager`, com JPA puro.

At the end desse laboratório você deverá ser capaz de:
...
### Task 1: Adding the dependencys do JPA
...
### Task 3: Criando uma Entity do JPA
...
### Task 4: Integrando JPA ao Service

Adapte `CarServiceImpl` para usar `EntityManager` e mapear entre `CarDTO` e `CarEntity`.

In this lab vamos usar uma classe intermediária (um pequeno DAO JPA) que encapsula o `EntityManager` e já é responsável por iniciar/participar de transações. Em seguida, atualizaremos `CarServiceImpl` para delegar para essa classe.
...
In this lab você adicionou H2, implementou um DAO com `JdbcTemplate` e criou uma camada JPA usando `EntityManager`. Próximos tópicos recomendados:
